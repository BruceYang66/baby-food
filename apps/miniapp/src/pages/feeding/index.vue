...
const savedTodayPlan = computed(() => todayPlan.value?.isSaved ? todayPlan.value : null)
...
<template>
  <view class="page-shell feeding-page">
    <AppNavBar title="辅食中心" subtitle="计划、食谱、收藏都在这里" :show-back="true" :silent-back-fallback="true" />
    ...
    <view v-else class="logged-view">
      <view v-if="savedTodayPlan" class="hero-card card">
        <view class="hero-main">
          <text class="hero-kicker">今日计划</text>
          <text class="hero-title">{{ savedTodayPlan.dateLabel }}</text>
          <text class="hero-desc">共 {{ savedTodayPlan.entries.length }} 餐，营养评分 {{ savedTodayPlan.nutritionScore }}，喝水建议 {{ savedTodayPlan.waterSuggestion }}</text>
        </view>
        <view class="hero-side">
          <text class="hero-badge">已保存</text>
          <view class="hero-btn primary-button" @tap="goPlanCenter">查看详情</view>
        </view>
      </view>

      <view v-else class="hero-card card empty-hero-card">
        <view class="hero-main">
          <text class="hero-kicker">今日计划</text>
          <text class="hero-title">今天还没有已保存的辅食计划</text>
          <text class="hero-desc">先生成并保存今日辅食计划，再回到这里统一查看和导入真实餐次。</text>
        </view>
        <view class="hero-side">
          <text class="hero-badge ghost">待生成</text>
          <view class="hero-btn primary-button" @tap="goGeneratePage">去生成</view>
        </view>
      </view>
      ...
      <view class="section">
        <view class="section-row">
          ...
          <view class="section-link" @tap="goPlanCenter">
      ...
      <view class="section">
        <view class="section-row">
          ...
          <view class="section-link" @tap="goPlanCenter">
            <text class="section-link-text">进入历史</text>
            <text class="section-link-arrow">›</text>
          </view>
        </view>
...
.empty-hero-card {
  background: linear-gradient(135deg, rgba(247, 239, 230, 0.72), rgba(255, 255, 255, 0.98));
}
...
.quick-card.accent {
  background: linear-gradient(135deg, rgba(255, 191, 214, 0.28), rgba(255, 255, 255, 0.96));
}
